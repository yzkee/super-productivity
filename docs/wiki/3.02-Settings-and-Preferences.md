...
When at least one background image is set, the background contrast slider is
shown. When at least one mode has no background image, the colored background
tint setting is shown. See [[3.09-Theming]] for theme details.
