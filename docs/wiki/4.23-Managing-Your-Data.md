...
**Automatic backups** — When enabled in settings, the app creates a backup on a schedule (e.g. every 5 minutes). Where it is stored depends on the platform: on **desktop (Electron)**, backups are timestamped JSON files in the User Data Folder (e.g. a `backups/` directory) with automatic cleanup of older files. On **Android** and **iOS**, backups are stored in platform-appropriate storage (e.g. IndexedDB or a dedicated file). On the **web**, there is no automatic file-based backup; you use “Export data” to download a file. See [[3.06-User-Data]] for exact locations and behavior per platform.
