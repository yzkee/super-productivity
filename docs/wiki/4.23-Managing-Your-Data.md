...
The app uses **local-first, operation-based sync**: your device is the primary copy; sync sends **changes** (operations) to the remote side (or a single sync file) rather than uploading the entire dataset every time. That keeps sync fast and bandwidth low. Multiple **sync providers** are supported (e.g. SuperSync server, WebDAV, Dropbox, local file); file-based providers typically use a single sync file that contains a state snapshot plus recent operations.
