...
The "Decryption Failed" dialog shown after a failed sync never offers to overwrite the server. It leaves the server copy untouched, and with SuperSync the part of your data that could be decrypted is kept on the device. The encryption password prompt refuses to overwrite the server when the device has never completed a sync and holds no data of its own; if you see that refusal, the server copy is untouched.
