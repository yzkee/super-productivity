...
| `parentId`           | Create the task as a subtask of this top-level task. Returns 404 if parent missing, 400 if parent is itself a subtask. The new subtask inherits the parent's `projectId`; any supplied `projectId`/`tagIds` are ignored by the reducer. |
