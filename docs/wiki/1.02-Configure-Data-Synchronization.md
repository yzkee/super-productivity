...
- **Multiple devices:** You need **one** sync provider. Each device can use a different provider, but typically you choose one (e.g. Dropbox or Nextcloud) and use it on all devices so data stays in sync.

---

## Step 1: Choose a Sync Provider

The app supports five options. Pick the one that matches how you work:

| If you want…                                                     | Choose               |
| ---------------------------------------------------------------- | -------------------- |
| Real-time sync and can accept beta software                      | **SuperSync (Beta)** |
| Cloud sync without running your own server                       | **Dropbox**          |
| To use your existing Nextcloud server                            | **Nextcloud**        |
| To use another WebDAV server                                     | **WebDAV**           |
| Backup or sync to a folder on this device (desktop/Android only) | **Local file**       |

**Do not** use file-syncing tools (e.g. Syncthing, Resilio) to sync the **Local file** folder between devices—that can cause conflicts. For multi-device sync, use Nextcloud, WebDAV, Dropbox, or SuperSync instead.
...
- **Nextcloud:** Enter **Server URL**, **Username**, and **App Password**. Use **Login name / email (optional)** only if your login differs from your username. Set **Sync Folder Path** if needed. Sync interval and manual-only apply; encryption is under **Advanced**.
- **WebDAV:** Enter **Base URL**, **Username**, and **Password**. Set **Sync Folder Path** if needed. Sync interval and manual-only apply; encryption is under **Advanced**.
...
If something fails, check the provider’s requirements (e.g. Nextcloud username vs login name, WebDAV URL format, CORS for web app) in [[2.09-Configure-Sync-Backend]] and [[3.08-Sync-Integration-Comparison]].
