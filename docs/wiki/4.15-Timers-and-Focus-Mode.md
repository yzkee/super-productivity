...
Focus Mode has **multiple dedicated settings** (e.g. sync with task tracking, start in background, skip preparation screen, play tick sound, pause task tracking during break, enable Pomodoro overtime). Those options show that focus is meant to be **configurable** to your workflow. The feature is also **woven into the app**: session lifecycle (start, tick, complete), break handling, task-tracking sync, and metrics all depend on focus state. So focus is **central to the workflow**, not a side panel you can ignore.
