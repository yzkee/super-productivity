...
When upgrading an existing single-provider setup, the old authorization is moved to that provider entry and the shared credential is removed. If there is no unambiguous existing provider, reconnect each entry instead; a newly added entry never inherits an old account's authorization. Disconnecting or deleting an entry removes its local credential, including when deletion arrives through sync. Cancelling a new entry also removes any authorization created for that unsaved entry.
