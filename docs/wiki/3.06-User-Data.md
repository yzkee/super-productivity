...
| **Android**            | IndexedDB keys `backup` and `backup_prev` (latest and previous backup slots) |
...
On Android, **Restore latest automatic backup** is available in Settings -> Sync & Backup -> Automatic Backups. It restores the newest usable automatic backup slot after confirmation and replaces the current local data. If sync is configured, the restored data is treated as a backup import: on the next sync it propagates to and overwrites your other synced devices, so use it as a recovery action rather than a routine one.
