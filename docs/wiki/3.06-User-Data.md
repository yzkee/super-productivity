...
- Watched while the app runs: creating, editing, or replacing the file re-applies it to the open window without a restart. Emptying or deleting the file removes the styles from the open window right away.
...
The retired conflict journal database (`SUP_CONFLICT_JOURNAL`) and its local
clear marker are deleted on startup without export. This removes only old
review entries; tasks, pending operations and backups are retained. If an older
tab holds the database open, cleanup completes after that connection closes.
...
### Legacy Local Data Migration Retirement

Automatic in-place migration of local data from **Super Productivity v16 or older** (the legacy `pf` database) to the operation-log store still works today. It will retire in the **first release after January 23, 2027**.

After retirement, if your local data has not yet been migrated, first run the last release that supports this migration, let it finish, and then upgrade to the latest release. That bridge release's version and download instructions will be added when it is known. Data already migrated to the operation-log store needs no bridge step.

This retirement applies to local data on a device, including browser storage. **Supported older JSON backup imports remain supported** through Import from file; the existing unsupported V1 format exception above still applies. Remote sync formats are separate and are not retired by this notice. See [[2.02-Restore-Data-From-Backup]] for JSON restore instructions.
...
**Conflict resolution:** Conflicts are detected using vector clocks (same entity changed concurrently in two places). Resolution first applies explicit archive/delete precedence and safe disjoint-field merging. Remaining overlaps use **last-write-wins (LWW)**: the newer operation timestamp wins; an exact tie uses stable client-ID ordering. When the local or merged result must dominate both histories, the app emits the appropriate update, archive-win, or delete-win operation for the next upload. **Identical conflicts** are auto-resolved without user action. The app may show a dialog to choose **Keep local** or **Keep remote** in two situations: **first-sync**, where both local and remote already have data, and when automatic resolution stops on changes it cannot merge safely (`SYNC_MULTI_ENTITY_UNSUPPORTED`) — see [[3.08-Sync-Integration-Comparison]]. Either choice replaces the entire losing dataset, not just the conflicting entities. After resolution, state is validated and repaired as needed.
