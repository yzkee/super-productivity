...
**Newer file formats:** If a remote sync file uses a newer format than this app supports, sync stops and asks you to update. The app does not recover that file from an older backup.

**Changes waiting during sync:** If too many local changes accumulate while remote changes are being applied, a persistent message warns that those changes have not been saved yet. It closes when the waiting changes are saved. Its **Reload** action discards the waiting changes.

**Android Nextcloud/WebDAV upload timeouts:** An upload may finish on the server after the app times out. The app reports the timeout without immediately replaying the upload; a later sync checks the remote state.
...
- **Concurrent writes:** Atomic conflict protection requires strong ETags and a server that honors `If-Match`. The app also checks the revision before uploading, but this is only best-effort protection on servers that ignore write preconditions or provide weak or missing ETags.
...
- **Large backlogs:** Regular downloads process large backlogs in passes, saving progress after each pass and scheduling the next one automatically. Full-history recovery downloads still require the complete history within one pass.
...
New file-sync configurations discover the folder's format. Empty folders use v3
split files (`sync-ops.json` and its referenced snapshot); joining an existing
v2 folder keeps `sync-data.json` in v2. Existing saved format settings are
preserved. **Surgical sync** remains an explicit, one-way upgrade of a v2 folder.
Older app versions may require enabling Surgical sync to join a v3 folder.
...
Logical conflict resolution (e.g. when two devices change the same data) uses the same operation rules for all providers. Transport guarantees still differ: Local file is single-writer only, and WebDAV atomic write protection requires strong ETags and enforcement of write preconditions. See [[4.23-Managing-Your-Data]].

### When Automatic Conflict Resolution Stops

Reordering notes, habits, boards or sections can resolve automatically alongside supported edits on another device: note text, a habit's daily count, board configuration or a section title. Both devices keep the content edit and converge on an order. Other overlapping changes, or missing history needed to recover safely, can still stop sync. Older app versions may also stop on these reorder conflicts.

If sync stops with `SYNC_MULTI_ENTITY_UNSUPPORTED`, choose **Resolve…** in the error message or start a manual sync to open the recovery dialog. Background retries do not open the dialog automatically.

- **Keep local** replaces the remote dataset with this device's data. Remote-only changes are lost.
- **Keep remote** replaces this device's dataset with the remote data. Unsynced local changes are lost.
- **Cancel** leaves the conflict unresolved and sync stopped.

For file-based sync, keep all devices updated when using **Keep local** or **Force overwrite**. Current clients retain the replacement boundary through later uploads so other devices load the replacement before its later changes. Older writers can omit that boundary, allowing an offline device to miss the replacement.

This is a recovery fallback; it does not fix the underlying overlapping changes, so the conflict can recur. These choices replace the entire dataset, not just the conflicting tasks. Export backups from devices with changes you want to preserve before choosing a version. Remote change counts and timestamps are shown as unknown when unavailable.
