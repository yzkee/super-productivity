...
Spell checking (red underlines for misspelled words) is intentionally disabled in the desktop (Electron) app on every operating system. Chromium's built-in spellchecker downloads its dictionaries from Google's CDN while the app runs, which conflicts with the app's offline-first, zero-data-collection approach, so the desktop app turns the spellchecker off at startup. The setting is reapplied on every launch, which is why editing Electron's internal `Preferences` file to re-enable it does not stick.
...
Re-enabling spell checking in the desktop app is not currently configurable; doing it without the dictionary downloads would require shipping or self-hosting spellcheck dictionaries. If spell checking matters to your workflow, please describe it in an issue.
