...
describe('MobileNotificationEffects', () => {
  ...
  describe('on native platform — disableReminders gating', () => {
    ...
    let cfg$: BehaviorSubject<any>;
    let store: MockStore;

    const buildCfg = (overrides: Partial<any> = {}): any => ({
      reminder: {
    ...
    beforeEach(() => {
      ...
      cfg$ = new BehaviorSubject<any>(buildCfg());
    ...
    const futureReminder = (id: string): any => ({
      id,
      title: `task ${id}`,
    ...
    it('schedules reminders normally when disableReminders is false', fakeAsync(() => {
      store.overrideSelector(selectAllTasksWithReminder, [futureReminder('a')] as any);
    ...
    it('cancels previously-scheduled reminders when disableReminders flips true', fakeAsync(() => {
  ...
  describe('on native platform — due-date gating', () => {
    ...
    const futureDueTask = (id: string): any => {
