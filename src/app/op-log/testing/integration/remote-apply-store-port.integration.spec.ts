...
/** Exercises composed apply/checkpoint/clock transactions on IndexedDB. */
describe('RemoteOperationApplyStorePort Integration (IndexedDB)', () => {
  ...
  it('should keep a reducer-failed full-state operation pending and recoverable', async () => {
    ...
    const applier: OperationApplyPort<Operation> = {
      applyOperations: async (_ops, options) => {
        await options?.onReducersCommitted?.([], [{ op: importOp, error: reducerError }]);
        return {
          appliedOps: [],
