...
10. **TODAY_TAG is a Virtual Tag**: TODAY_TAG (ID: `'TODAY'`) must **NEVER** be added to `task.tagIds`. It's a "virtual tag" where membership is determined by `task.dueDay`, and `TODAY_TAG.taskIds` only stores ordering. This keeps move operations uniform across all tags. See `ARCHITECTURE-DECISIONS.md` Decision #2.
...
13. **Vector Clock Pruning**: `MAX_VECTOR_CLOCK_SIZE` is 20 (a 20-entry clock is ~333 bytes). Pruning is extremely rare (needs 21+ unique client IDs). The server MUST prune (`limitVectorClockSize`) **after** conflict detection but **before** storage — never before comparison. The server's `sanitizeVectorClock()` applies a DoS cap at `2.5x MAX_VECTOR_CLOCK_SIZE` (50 entries) but does NOT prune to MAX. See `docs/sync-and-op-log/vector-clocks.md`.
14. **Logical clock**: For "what day does this moment belong to?", go through `DateService` (`getLogicalTodayDate`, `getLogicalTomorrowMs`, `todayStr`, `isToday`). Pure reducers and selectors take `startOfNextDayDiffMs` as an explicit argument and call `isTodayWithOffset` — this is required for sync-replay determinism. The raw `DateService.startOfNextDayDiff` field is `private`, so TypeScript prevents direct access; use `getStartOfNextDayDiffMs()` if you need the raw value at a service boundary.
